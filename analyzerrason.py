...
#         # ambil metadata semua stasiun (ICAO → info dict)
#         stations = await fetch_all_stations_info(token, session)

#         # Fetch RASON: type_message=3
#         rason_data = await fetch_gts_data(token, session, 2025, 9, type_message=3)

#         # Cek cepat jumlah record
#         print(f"\nTotal RASON record: {len(rason_data)}")

#         # Cek apakah ada record untuk Soekarno-Hatta (WMO: 96749, ICAO: WIII)
#         wmo_target = "96749"
#         wiii_records = [r for r in rason_data if str(r.get("station_wmo_id")) == wmo_target]
#         if wiii_records:
#             print(f"Ada {len(wiii_records)} record RASON untuk Soekarno-Hatta (WMO {wmo_target})")
#             print("Contoh 1 record:")
#             print(wiii_records[0])
#         else:
#             print(f"Tidak ada record RASON untuk Soekarno-Hatta (WMO {wmo_target})")

#         # Analisis
#         df_rason_harian, df_rason_bulanan = analyze_rason(
#             rason_data, stations, 2025, 9
#         )

#         # Contoh hasil untuk Soekarno-Hatta
#         print("\nContoh rekap harian Soekarno-Hatta:")
#         print(df_rason_harian[df_rason_harian["WMO ID"] == wmo_target].head(1))

#         print("\nContoh rekap bulanan Soekarno-Hatta:")
#         print(df_rason_bulanan[df_rason_bulanan["WMO ID"] == wmo_target].head(1))

#         # Hitung total stasiun unik dari seluruh raw data
#         all_stasiun = {str(r.get("station_wmo_id")) for r in rason_data if r.get("station_wmo_id")}
#         print(f"Total stasiun RASON secara keseluruhan: {len(all_stasiun)}")

#         print("Contoh station_info_map:")
#         print(list(stations.items())[:3])  # print 3 stasiun pertama
        
#         print(f"Total stasiun dari station_info_map: {len(stations)}")
